...
export default function Banner({ movie }: { movie: Movie; }) {
	const { bannerFileURL, title, description, videoFileURL, id } = movie;
	return (
			...
			<div className='flex flex-col space-y-4 py-16 md:space-y-4 lg:h-[65vh] lg:justify-end lg:pb-12'>
				<div className='absolute left-0 top-0 -z-10 flex h-[95vh] w-screen flex-col bg-black'>
					<video
						...
						loop
						muted
						className="z-20 hidden h-full object-cover opacity-30 transition duration-1000 ease-in-out lg:block"
						poster={bannerFileURL}
						src={videoFileURL}
					/>
					<Image
						src={bannerFileURL}
				...
				<p className='text-shadow-md max-w-xs text-xs md:max-w-lg md:text-lg lg:max-w-2xl'>{description}</p>
			</div>
